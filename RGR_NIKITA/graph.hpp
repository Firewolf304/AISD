...
#define INF 9999999
    ...
    bool ready = false;
public:
    jhonson(std::shared_ptr<graph<VertexT, EdgeT>> g) : g(g) { }
    jhonson(const jhonson* hamil) {
        this->g = hamil->g;
    }
    /*void print_result() {
        if(ready) {
    ...
    bool check() {
        ...
        johnsonAlgorithm(matrix, distMatrix);

        int u = this->g->getIndex(this->g->getVertexFromName(value1)),v = this->g->getIndex(this->g->getVertexFromName(value2));
        std::vector<int> shortestPath = findShortestPath(u, v, distMatrix);
        std::cout << "Shortest path from " << u << " to " << v << " is: ";
        if (shortestPath[v] == INF) {
            std::cout << "No path exists." << std::endl;
        } else {
            std::cout << shortestPath[v] << std::endl;
        }
        ready = true;
        return ready;
    }
private:
    ...
    void bellmanFord(int vertices, int source, const std::vector<std::vector<int>>& g, std::vector<int>& dist) {
        dist[source] = 0;
        std::vector<std::tuple<int, int, int>> edges;
        for (int u = 0; u < vertices; ++u) {
            for (int v = 0; v < vertices; ++v) {
                if (g[u][v] != INF) {
                    edges.emplace_back(u, v, g[u][v]);
                }
            }
        }
    ...
    void johnsonAlgorithm(const std::vector<std::vector<int>>& g, std::vector<std::vector<int>>& distMatrix) {
        int vertices = g.size();
        std::vector<std::vector<int>> newGraph(vertices + 1, std::vector<int>(vertices + 1, INF));

        // Copy the original graph into newGraph and add new vertex s
        for (int u = 0; u < vertices; ++u) {
            for (int v = 0; v < vertices; ++v) {
                newGraph[u][v] = g[u][v];
            }
            newGraph[vertices][u] = 0; // Edge from new vertex to all other vertices
        }

        // Step 2: Run Bellman-Ford algorithm from the new vertex s to find h(v) for all v
        std::vector<int> h(vertices + 1, INF);
        try {
            bellmanFord(vertices + 1, vertices, newGraph, h);
        } catch (const std::runtime_error& e) {
            std::cout << e.what() << "\n";
            return;
        }

        // Step 3: Reweight the original edges using h(v)
        std::vector<std::vector<std::pair<int, int>>> adj(vertices);
        for (int u = 0; u < vertices; ++u) {
            for (int v = 0; v < vertices; ++v) {
                if (g[u][v] != INF) {
                    int weight = g[u][v] + h[u] - h[v];
                    adj[u].emplace_back(v, weight);
                }
            }
        }

        // Step 4: Run Dijkstra's algorithm from each vertex to find the shortest paths
        distMatrix.resize(vertices, std::vector<int>(vertices, INF));
        for (int u = 0; u < vertices; ++u) {
            dijkstra(u, adj, distMatrix[u]);
            for (int v = 0; v < vertices; ++v) {
                if (distMatrix[u][v] != INF) {
                    distMatrix[u][v] = distMatrix[u][v] - h[u] + h[v];
                }
            }
        }
    }
    std::vector<int> findShortestPath(int u, int v, const std::vector<std::vector<int>>& g) {
        int vertices = g.size();
        ...
        while (!pq.empty()) {
            ...
            for (int i = 0; i < vertices; ++i) {
                if (g[curr][i] != INF && dist[curr] + g[curr][i] < dist[i]) {
