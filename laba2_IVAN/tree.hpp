//
// Created by firewolf on 07.09.24.
//

#ifndef LABA2_IVAN_TREE_HPP
#define LABA2_IVAN_TREE_HPP
//
...
public:
    ...
    int getOrder(tk key) {                                                                                  // получение порядкового номера
        auto value = _getOrder_length(key, this->root);
        return value;
    }
    ...
    void print_reverse(int level = 0)                                                              // вывод дерева
    {
        _print_reverse(this->root, level);
    }
...
private:
    ...
    bool _remove(const tk & key, int & hops = def )
    {
        try {
            ...
            Root->right = nullptr;
            Root.reset();
            _update_under_size(key, this->root, hops);
            return true;
    ...
    bool _add(const tk & key, const td & obj, std::shared_ptr<Node> & subtree, int & hops, const int & size = 0)
    {
        hops++;
        if(subtree == nullptr) {
            subtree = std::make_shared<Node>(key, obj);
            subtree->length = size;
            if(this->maxLevel < size) {
                this->maxLevel = size;
            }
            return true;
        } else {
    ...
    void _print_reverse(std::shared_ptr<Node> & subtree, int level = 0)                            // рекурсивный вывод поддерева
    {
        /*if(subtree == nullptr) {
            return;
        }
        ...
        if(subtree->left != nullptr)
            _print_reverse(subtree->left, level+1);
        std::cout << std::string( this->maxLevel - level, '\t') << subtree->item << std::endl;
        if(subtree->right != nullptr)
            _print_reverse(subtree->right, level+1);
...
public:
    ...
    public:
        ...
        const std::shared_ptr<Node>& _data(){
            if(this->current == nullptr)
                throw std::runtime_error("Index out of bands");
            return this->current;
        }
