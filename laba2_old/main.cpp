...
int main() {
    ...
    std::unordered_map<std::string, std::function<void()>> map = {
        ...
        {"reset - удаление поддерева по ключу", [&my_tree]()->void {
            std::string value;
            std::cout << "Insert int key: ";
            std::getline(std::cin >> std::ws, value);
            std::cout << "RETURN: " << (my_tree.reset(std::atoi(value.c_str())) ? "true" : "false") << std::endl;
        }},
