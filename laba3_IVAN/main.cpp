...
int main() {
    ...
    std::unordered_map<std::string, std::function<void()>> map = {
            ...
            {"add - добавление поддерева", [&my_tree]()->void {
                ...
                my_tree.add(std::atoi(key.c_str()),std::atoi(value.c_str()), inserted);
