...
int main() {
    ...
    std::shared_ptr<FindShortWay<Vertex<std::string, int>, Edge<Vertex<std::string, int>,int, int>>> hamil;
    std::shared_ptr<Hamiltonian<Vertex<std::string, int>, Edge<Vertex<std::string, int>,int, int>>> rgr2;
    ...
    std::map<std::string, std::function<void()>> mapper = {
        ...
        {"Определение кратчайших путей", [&]() {
            std::cout << "1. Обработка кратчайших путей" << std::endl;
            std::cout << "2. Вывод путей" << std::endl;
            std::cout << "3. Вывод путей между парой вершин" << std::endl;
            int command;
            std::cin >> command;
            switch(command) {
                case 1: {
                    std::cout << "Result: " << (hamil->check() ? "true" : "false") << std::endl;
                } break;
                case 2: {
                    hamil->print_result();
                } break;
    ...
    hamil = std::make_shared<FindShortWay<Vertex<std::string, int>, Edge<Vertex<std::string, int>,int, int>>>(grapher);
    rgr2 = std::make_shared<Hamiltonian<Vertex<std::string, int>, Edge<Vertex<std::string, int>,int, int>>>(grapher);
    while(true) {
        std::cout << "Commands:" << std::endl;
