...
public:
    ...
    void print_result() {
        if(ready) {
            std::cout << "Матрица кратчайших путей по числу ребер между всеми парами вершин:" << std::endl;
    ...
    void result(std::string a, std::string b) {
        if(ready) {
            auto v1 = g->getVertexFromName(a);
            auto v2 = g->getVertexFromName(b);
            int indexA = g->getIndex(v1);
            int indexB = g->getIndex(v2);
            if (dist[indexA][indexB] == INF) {
                std::cout << "No path exits" << std::endl;
            } else {
                std::cout << v1->getName() << " ";
                while (indexA != indexB) {
                    /*for(int i = 0; i < dist.size(); i++) {
                        if(dist[indexA][indexB] == dist[indexA][i] + 1 && dist[i][indexB] == 1) {
                            std::cout << i << " ";
                            indexA = i;
                            break;
                        }
                    }*/
                    indexA = next[indexA][indexB];
                    std::cout << g->getVertex(indexA)->getName() << " ";
                }
                std::cout << std::endl;
...
private:
    ...
    void floydWarshall(std::vector<std::vector<int>>& g) {
        ...
        this->next = std::vector<std::vector<int>>(n, std::vector<int>(n, 0));
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                /*if (g[i][j] != 0) {
                    dist[i][j] = 1;
...
private:
    ...
    void FloydWarshall(std::vector<std::vector<int>>& g) {
        ...
        int minMaxDist = INF;
        std::vector<int> peripheryVertices;
        ...
        if (!peripheryVertices.empty()) {
            std::cout << "Periphery vertex is " << std::endl;
            for (int vertex : peripheryVertices) {
                std::cout << this->g->getVertex(vertex)->getName() << " ";
            }
            std::cout  << std::endl;
        } else {
            std::cout << "Graph has no periphery" << std::endl;
        }
    }
