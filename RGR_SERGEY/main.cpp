...
int main() {
    ...
    std::shared_ptr<Color<Vertex<std::string, int>, Edge<Vertex<std::string, int>,int, int>>> color;
    ...
    std::map<std::string, std::function<void()>> mapper = {
        ...
        {"Задание 2", [&]() {
            std::cout << "1. Нахождение кратчайшего пути" << std::endl;
    ...
    color = std::make_shared<Color<Vertex<std::string, int>, Edge<Vertex<std::string, int>,int, int>>>(grapher);
    while(true) {
        std::cout << "Commands:" << std::endl;
